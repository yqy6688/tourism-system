...
public class UserService {
    ...
    public void updateUser(Long id, User user) {
        ...
        // 同时清除JwtTokenUtils中的用户缓存
        String jwtUserCacheKey = "user:id:" + id;
        redisUtil.del(jwtUserCacheKey);
    ...
    public void updatePassword(Long id, UserPasswordUpdateDTO update) {
        ...
        //新旧密码一样
        if (update.getOldPassword().equals(update.getNewPassword())) {
            throw new ServiceException("新旧密码不能相同");
        }
        // 更新新密码
    ...
    public Map<String, Object> getUserStats(Long userId) {
        ...
        try {
            // 统计发布攻略数量
            LambdaQueryWrapper<org.example.springboot.entity.TravelGuide> guideWrapper = new LambdaQueryWrapper<>();
            guideWrapper.eq(org.example.springboot.entity.TravelGuide::getUserId, userId);
            Long guideCount = travelGuideMapper.selectCount(guideWrapper);
            stats.put("guides", guideCount);
            
            // 统计收藏景点数量
            LambdaQueryWrapper<org.example.springboot.entity.ScenicCollection> collectionWrapper = new LambdaQueryWrapper<>();
            collectionWrapper.eq(org.example.springboot.entity.ScenicCollection::getUserId, userId);
            Long collectionCount = scenicCollectionMapper.selectCount(collectionWrapper);
            stats.put("collections", collectionCount);
            
            // 统计订单总数（门票订单 + 酒店订单 + 餐饮订单）
            LambdaQueryWrapper<org.example.springboot.entity.TicketOrder> ticketOrderWrapper = new LambdaQueryWrapper<>();
            ticketOrderWrapper.eq(org.example.springboot.entity.TicketOrder::getUserId, userId);
            Long ticketOrderCount = ticketOrderMapper.selectCount(ticketOrderWrapper);
            
            LambdaQueryWrapper<org.example.springboot.entity.AccommodationOrder> accommodationOrderWrapper = new LambdaQueryWrapper<>();
            accommodationOrderWrapper.eq(org.example.springboot.entity.AccommodationOrder::getUserId, userId);
            Long accommodationOrderCount = accommodationOrderMapper.selectCount(accommodationOrderWrapper);
            
            LambdaQueryWrapper<org.example.springboot.entity.RestaurantOrder> restaurantOrderWrapper = new LambdaQueryWrapper<>();
            restaurantOrderWrapper.eq(org.example.springboot.entity.RestaurantOrder::getUserId, userId);
            Long restaurantOrderCount = restaurantOrderMapper.selectCount(restaurantOrderWrapper);
            
            Long totalOrders = ticketOrderCount + accommodationOrderCount + restaurantOrderCount;
            stats.put("orders", totalOrders);
            
            // 统计待使用门票数量（已支付但未使用的门票订单）
            LambdaQueryWrapper<org.example.springboot.entity.TicketOrder> pendingTicketWrapper = new LambdaQueryWrapper<>();
            pendingTicketWrapper.eq(org.example.springboot.entity.TicketOrder::getUserId, userId)
                              .eq(org.example.springboot.entity.TicketOrder::getStatus, 1); // 1表示已支付
            Long pendingTicketCount = ticketOrderMapper.selectCount(pendingTicketWrapper);
            stats.put("tickets", pendingTicketCount);
            
            logger.info("用户{}统计数据：攻略={}, 收藏={}, 订单={}, 待使用门票={}", 
                userId, guideCount, collectionCount, totalOrders, pendingTicketCount);
                
        } catch (Exception e) {
            logger.error("获取用户{}统计数据失败", userId, e);
            // 如果查询失败，返回默认值
            stats.put("guides", 0);
     ...
     * 检查商家营业资格证审核状态
     * @param userId 商家用户ID
     * @return 审核状态：PENDING(待审核)、APPROVED(通过)、REJECTED(拒绝)、NOT_SUBMITTED(未提交)
     */
    public String checkMerchantLicenseStatus(Long userId) {
        try {
            var license = businessLicenseService.getLicenseByUserId(userId);
            if (license == null) {
                return "NOT_SUBMITTED";
            }
            return license.getStatus();
        } catch (Exception e) {
            logger.error("检查商家营业资格证状态失败", e);
            return "NOT_SUBMITTED";
        }
    }
