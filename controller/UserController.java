...
public class UserController {
    ...
    @PostMapping("/add")
    public Result<?> createUser(@RequestBody  Map<String, Object> userMap) {
        try {
            // 先处理偏好标签，避免转换时出错
            Object preferences = userMap.remove("preferences");
    ...
    public Result<?> registerByEmail(@RequestBody Map<String, Object> userMap, @RequestParam String verifyCode) {
        // 验证参数
        if (!userMap.containsKey("email") || !userMap.containsKey("password") || 
            !StringUtils.hasText(userMap.get("email").toString()) || !StringUtils.hasText(userMap.get("password").toString())) {
            return Result.error("邮箱和密码不能为空");
        }
    ...
    @Operation(summary = "获取商家营业资格证审核状态")
    @GetMapping("/merchant-license-status/{userId}")
    public Result<?> getMerchantLicenseStatus(@PathVariable Long userId) {
        try {
            String status = userService.checkMerchantLicenseStatus(userId);
            return Result.success(status);
        } catch (Exception e) {
            LOGGER.error("获取商家营业资格证审核状态失败", e);
            return Result.error("获取审核状态失败：" + e.getMessage());
        }
    }
