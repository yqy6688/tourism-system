...
public class AccommodationReviewController {
    ...
    public Result<?> getReviewsByPage(
        ...
        try {
            LOGGER.info("分页查询酒店评价，参数：accommodationId={}, page={}, size={}",
            ...
            LOGGER.error("查询酒店评价失败", e);
            return Result.error("查询酒店评价失败：" + e.getMessage());
        }
    }
    
    @Operation(summary = "添加酒店评价")
    @PostMapping
    public Result<?> addReview(@RequestBody AccommodationReview review) {
        try {
            LOGGER.info("添加酒店评价：{}", review);
            ...
            if (result) {
                ...
                return Result.error("添加酒店评价失败");
            }
        } catch (Exception e) {
            LOGGER.error("添加酒店评价失败", e);
            return Result.error("添加酒店评价失败：" + e.getMessage());
        }
    }
    
    @Operation(summary = "删除酒店评价")
    @DeleteMapping("/{id}")
    public Result<?> deleteReview(@PathVariable Integer id) {
        try {
            LOGGER.info("删除酒店评价，id={}", id);
            ...
            if (result) {
                ...
                return Result.error("删除酒店评价失败，可能无权限或评价不存在");
            }
        } catch (Exception e) {
            LOGGER.error("删除酒店评价失败", e);
            return Result.error("删除酒店评价失败：" + e.getMessage());
        }
    }
} 